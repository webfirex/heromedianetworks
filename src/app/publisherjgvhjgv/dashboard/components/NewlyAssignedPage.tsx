...
import { IconGift, IconCoin, IconExternalLink, IconMapPin, IconInfoCircle, IconCalendar } from '@tabler/icons-react';
...
export default function NewlyAssignedPage() {
  ...
  useEffect(() => {
    const fetchOffers = async () => {
      ...
      try {
        ...
        if (!publisher_id) {
          setError('You must be logged in as a publisher to view assigned offers.');
          setLoading(false);
          return;
        }
        const res = await fetch(`/api/offers/newly-assigned?publisher_id=${publisher_id}`);
        if (!res.ok) throw new Error('Failed to fetch newly assigned offers');
        const data = await res.json();
        setOffers(data);
      } catch (e: unknown) {
        const msg = e instanceof Error ? e.message : 'Failed to fetch newly assigned offers';
        setError(msg);
        showNotification({
          title: '⚠️ Error',
          message: msg,
  ...
  return (
    <Card shadow="md" radius="md" withBorder>
      {offers.length === 0 ? (
        <Text>No newly assigned offers found.</Text>
      ) : (
        <Table striped highlightOnHover withTableBorder>
          <Table.Thead>
            <Table.Tr>
              ...
              <Table.Th><Group gap={4} style={{ fontSize: 15 }}><IconCalendar size={18} /> Assigned At</Group></Table.Th>
          ...
          <Table.Tbody>
            {offers.map((offer) => (
              <Table.Tr key={offer.id}>
                ...
                <Table.Td>
                  <Badge color="gray" variant="light" style={{ fontSize: 14 }}>{new Date(offer.assigned_at).toLocaleString()}</Badge>
                </Table.Td>
                <Table.Td>
